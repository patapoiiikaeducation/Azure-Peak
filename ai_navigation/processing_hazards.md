...
- blocking calls in process-heavy code:
  `rg -n "\binput\(|\balert\(|browse\(|do_after\(|sleep\(|waitfor = TRUE|set waitfor = TRUE" code modular -g "*.dm"`
- walk instructions used without spawn (sleeps internally):
  `rg -n "\bwalk\b|\bwalk_to\b|\bwalk_towards\b|\bwalk_rand\b|\bwalk_away\b" code modular -g "*.dm"`
- async handoff:
  `rg -n "\bINVOKE_ASYNC\b|set waitfor = FALSE|set waitfor = 0|addtimer\(" code modular -g "*.dm"`
